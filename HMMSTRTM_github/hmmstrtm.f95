module hmmstrtm
        ...
        use drct2profile!, only : get_profile
        use gamma_io
        implicit none
        type(HMM) :: HMMSTR
...
subroutine scratch_hmmstr(modelfile, drctfile, code)
  ...
  do while(this_profile%nres /= -1)
          ...
          ! ---------------------------------------------------------------------------
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          call run_hmmstr(HMMSTR, tgamma, zeta, zeta_dict, training_flags, eval_flags)
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ! ---------------------------------------------------------------------------
...
subroutine run_hmmstr(HMMSTR, tgamma, zeta, zeta_dict, alg_flags, eval_flags)
  ...
  real,dimension(5),intent(inout) :: eval_flags
  ! mda stuff ! --------------------------------------- 9/8/21
  ...
  if ( unlabel_bool ) then ! 1/3/22 TMHMM TM unlabeling scheme
          call getalpha(ct,alpha,nres,HMMSTR,profile,intrans,outtrans,ramaseq,ssseq,tmseq_unlabel,alg_flags) 
          call getbeta(ct,beta,nres,HMMSTR,profile,intrans,outtrans,ramaseq,ssseq,tmseq_unlabel,alg_flags)
         ...
         call getalpha(ct,alpha,nres,HMMSTR,profile,intrans,outtrans,ramaseq,ssseq,tmseq,alg_flags) 
          call getbeta(ct,beta,nres,HMMSTR,profile,intrans,outtrans,ramaseq,ssseq,tmseq,alg_flags)
  ...
  !write(*,*) eval_flags
  ! ---------------------------------------------------------------------------
  if ( eval_flags(1) == 1. ) then
  !   if ( mda_acc ) then
          call viterbi(nres, HMMSTR, profile, intrans, outtrans, aaseq, ramaseq, ssseq, tmseq, alg_flags, paradigm )
          call get_mda(ground_truth_profiles, paradigm, mda, nres, tmda)
   !       write(*,*) " COLLECTED ", mda_weight_accs(sitr,:)
   endif
   if ( eval_flags(2) == 1. ) then
  !   if ( viterbi ) then
          call viterbi(nres, HMMSTR, profile, intrans, outtrans, aaseq, ramaseq, ssseq, tmseq, alg_flags, paradigm )
   endif
   if ( eval_flags(3) == 1. ) then
  !     write(*,*) "GETTING P(O|Model)"
       treal = 1
       do titr=1, nres
...
end subroutine run_hmmstr
        

! TLB 5/25/20
subroutine train_hmmstr(modelfile, drctfile, epochs) ! epochs = # iterations through training set.
  ! ------------------------------------------------------------------------
  implicit none
  ! ------------------------------------------------------------------------
  character(len=*),intent(inout) :: modelfile
  ...
  do ei=1, epochs
    ...
    do while(this_profile%nres /= -1)
      ...
      !do nitr=1,HMMSTR%n
      !  do nitr2=1,HMMSTR%n
      !     newA(nitr,nitr2) = ABS(newA(nitr,nitr2))
      !  enddo
      !enddo
      !write(*,*) "NEWA = ", newA
      ...
      !write(*,*) HMMSTR%a(:HMMSTR%n, :HMMSTR%n)
      !write(*,*) newA(:HMMSTR%n, :HMMSTR%n)
      write(*,*) "-----------------------------------------------------------------"
      !write(*,*) "OLD PRIORS = ", HMMSTR%priors
      !write(*,*) "NEW PRIORS = ", new_priors(:)
      write(*,*) "SUM OLD PRIORS = ", sum(HMMSTR%priors)
      write(*,*) "SUM NEW PRIORS = ", sum(new_priors(:))
         ...
         do itr=1, outtrans(nitr_i)%c(0)
              ...
              if (j_ind /= 0) then
                  do titr = 1, nres - 1
                      ...
                      zind = zeta_dict(nitr_i2,nitr_j2)
                      !write(*,*) nitr_i, nitr_j
                      !write(*,*) nitr_i2, nitr_j2
                      !write(*,*) zind, HMMSTR%a(nitr_i2,nitr_j2)
                      sum_zeta = sum_zeta + zeta(zind,titr)
                      call zeta_set(nitr_i,nitr_j,titr,zeta,zeta_dict,sum_zeta)
                      call zeta_set(nitr_i2,nitr_j2,titr,zeta,zeta_dict,sum_zeta)
                  enddo
              ...
              do titr=1, nres
                  ...
                  if ( bitr <= mrama ) then ! putting rama in same loop as residues cuts down runtime
                      if (bitr == index(rama_string, ramaseq(titr))) then
                          rpsab = rpsab + ab  ! rama will not have prob. dist like profile might... just add gamma * 1 
                      end if
                  end if
  ...
  real(8) function bprof_bg(iq,profile,HMMSTR)
   ...
   do a=1,m
     ...
     !  write(*,*) "STATE EMISSION = ", tmp, "GROUND TRUTH = ", profile(a)
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ! TSET 1 : * 1.5; 0.01 EPS; bp / ( 1 + bp ) 09/15/21 13:20, Mozart !!!
       ! P(O|Model) = +14.90; +2.50; +0.79
       ! MDA (TRAIN)= 54.50; 62.00; 62.15; 62.08; 61.98
       ! MDA (TEST) = 57.18; 62.57; 64.32; 64.12;
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ! TSET 2 : * 1.5; 0.001 EPS; bp / ( 1 + bp ) 09/15/21 13:00 , local !!
       ! P(O|Model) = +15.13; +2.54; +0.82; +0.35
       ! MDA (TRAIN)= 54.70; 62.21; 62.29; 62.14; 61.77 
       ! MDA (TEST) = 57.33; 62.28; 63.13; 63.25; 62.84
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ! TSET 3 : * 1.5; 0.0001 EPS; bp / ( 1 + bp ) 09/15/21 16:30 , local !!
       ! P(O|Model) = +15.12; +2.52;
       ! MDA (TRAIN)= 54.67; 62.16; 62.34; 62.11
       ! MDA (TEST) = 57.37; 62.63; 63.19; 64.51
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ! TSET 4 : 0.0001 EPS; bp / ( 1 + bp ) 09/15/21 16:50, Mozart !!
       ! P(O|Model) = +12.23; +1.52; +0.46
       ! MDA (TRAIN)= 55.47; 62.62; 62.35; 62.58; 62.28
       ! MDA (TEST) = 58.32; 64.54; 64.63; 64.55; 63.80
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ! TSET 5 : 0.001 EPS; bp / ( 1 + bp ) 09/15/21 21:10, Mozart !!
       ! P(O|Model) = +12.26; +1.55; +0.46; +0.22; +0.15  
       ! MDA (TRAIN)= 55.44; 62.69; 62.46; 62.25; 61.87
       ! MDA (TEST) = 58.14; 64.41; 64.74; 64.59; 63.94
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ! TSET 6 : * 1.5; 0.001 EPS 09/15/21 21:20, Local!!
       ! P(O|Model) = +6.07; +0.94; +0.36; +0.18; +0.14
       ! MDA (TRAIN)= 50.33; 44.87; 44.88; 44.87; 44.84
       ! MDA (TEST) = 51.22; 39.15; 39.68; 43.11; 43.22
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ! log-log ( worse ) TLB 9/12/21 !!!!!!!!!!!!!
       !tmp2 = log(((profile(a)+(EPS*HMMSTR%AA_ground(a)))/((1.+EPS)*HMMSTR%AA_ground(a)))) 
       !if ( tmp2 /= tmp2 .or. tmp2 / 2 == tmp2) cycle
       !logsum = logsum + tmp * tmp2
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   enddo
   !write(*,*) "LOGSUM = ", logsum
   ...
   !bprof_bg = exp(logsum)
   bprof_bg = logsum
   !bprof_bg = bprof_bg / ( 1 + bprof_bg ) 
   !write(*,*) bprof_bg 
   !if (bprof_bg <= 0.0) then
   if ( .false. ) then 
...
!-------------------------------------------------------------------------------------------------------------
   ...
   if (flags(2) == 1 .and. rama_c /= '?') then ! TLB 9.25 code review.. if rama_c == "?" then rama_p == 1 always. 
           ...
           if( k < mrama+1) then
              if (HMMSTR%RAMA_ground(k) /= 0) then
                  ...
                  ! RAMA_p = (HMMSTR%RAMA(iq,k) + EPS*HMMSTR%RAMA_ground(k))/((1.+EPS)*HMMSTR%RAMA_ground(k))
                 ! RAMA_p =HMMSTR%RAMA(iq,k)
          !        RAMA_p = exp(RAMA_p) 
          !        RAMA_p = RAMA_p / ( 1 + RAMA_p) 
              endif
           endif
   endif
   ! get SS_p
   if (flags(3) ==1) then
           ...
           if (k<mdssp+1) then
              if (HMMSTR%SS_ground(k) /= 0) then
                  ...
                  ! SS_p = (HMMSTR%SS(iq,k) + EPS*HMMSTR%SS_ground(k))/((1.+EPS)*HMMSTR%SS_ground(k))
  !                  SS_p = log ( HMMSTR%SS(iq,k) / ( 1 - HMMSTR%SS(iq,k) ) )
   !            SS_p = HMMSTR%SS(iq,k) 
              endif
           endif
   endif
   ...
   if (flags(4) == 1 .and. tm_c /= 'U') then
           ...
           if (k<mtm+1) then
              if (HMMSTR%TM_ground(k) /= 0) then
                 ...
                 !     TM_p = (HMMSTR%TM(iq,k) + EPS*HMMSTR%TM_ground(k))/((1.+EPS)*HMMSTR%TM_ground(k))
                    !TM_p = HMMSTR%TM(iq,k)
                !     TM_p = log ( HMMSTR%TM(iq,k) / ( 1 - HMMSTR%TM(iq,k) ) ) 
   !            TM_p = HMMSTR%TM(iq,k) 
              endif
           endif
   endif
  ...
  !tmp = AA_p*SS_p*TM_p*RAMA_p ! all probabilities multiplied together? 
  tmp = exp( AA_p + RAMA_p + SS_p + TM_p )
  !write(*,*) AA_p, SS_p, TM_p, RAMA_p, "=", tmp
  !write(*,*) flagrat_brdc_prod
  
  !flagrat_brdc_prod = ( tmp / ( 1 + tmp ) )
  flagrat_brdc_prod = tmp
  !write(*,*) flagrat_brdc_prod
  !if ( flagrat_brdc_prod > 1 .or.
  !if ( flagrat_brdc_prod < 0 .or.flagrat_brdc_prod /= flagrat_brdc_prod ) then 
  !if (TM_p < 1 ) then
  !if ( flagrat_brdc_prod > 0 )  then      !.or. (flagrat_brdc_prod < 0.001 .and. flagrat_brdc_prod > -0.001)) then 
  if ( .false. ) then
      c = c + 1
 ...
 ! write(*,*) AA_p, SS_p, TM_p, RAMA_p, "=", flagrat_brdc_prod
...
real recursive function alpha_naught_recurse(ind, titr, alpha, HMMSTR, intrs) result(a_ik)
  ...
  !------------------------------------------------------------------------------------------------------------
  subroutine getalpha(ct,alpha,seqres_len,HMMSTR,profile,intrs,outrs,ramaseq,ssseq,tmseq,flags)
   !--------------------------------------------------
   implicit none
   !--------------------------------------------------
   character(len=1),dimension(:),intent(in)::ramaseq
...
!-------------------------------------------------------------------------------------------------------------
subroutine getbeta(ct,beta,seqres_len,HMMSTR,profile,intrs,outrs,ramaseq,ssseq,tmseq,flags)
   !--------------------------------------------
   implicit none
   !--------------------------------------------
   character(len=1),dimension(:),intent(in)::ramaseq
   ...
   do nitr = 1, n
      beta(nitr, seqres_len) = ct(seqres_len) ! Fixed w/ scaled value ... see eqn (94)
   enddo  
      if ( allocated(memobt) .eqv. .false.) then
          allocate(memobt(HMMSTR%n))
          deallocate_flag = .true.
...
subroutine voting(aaseq1, tmseq1, ssseq1, ramaseq1, aa_prof, tm_prof, ss_prof, rama_prof, titr)
   implicit none
    ...
    real(8),dimension(5) :: rama_parties
    ...
    aaseq1(titr) = chr
    ! TM
    ind = maxloc(tm_prof,1)
    chr = tm_string(ind:ind)
    tmseq1(titr) = chr
    ! RAMA ! PRIMARIES:
    rama_parties(1) = rama_prof(1) + rama_prof(2) ! ( H, G )
    rama_parties(2) = rama_prof(3) + rama_prof(4) + rama_prof(5) + rama_prof(6) + rama_prof(7) ! ( B, E, d, b, e ) 
    rama_parties(3) = rama_prof(8) + rama_prof(9) ! ( L, l ) 
    rama_parties(4) = rama_prof(7) + rama_prof(10)  ! ( e, x ) 
    rama_parties(5) = rama_prof(11) ! ( c ) 
...
real(8) recursive function Viterbi_naught_traverse(ind, jnd, titr, HMMSTR, intrs) result(a_ik)
  ...
  subroutine viterbi(seqres_len, HMMSTR, profile, intrs, outrs, seq, ramaseq, ssseq, tmseq, flags, paradigm)
   ...
   do titr=seqres_len,1,-1
      ...
      call voting(out_seq, out_tmseq, out_ssseq, out_ramaseq, aa_prof, tm_prof, ss_prof, rama_prof, titr)
      !out_seq(titr) = HMMSTR%AA_seq(Q(titr))
      !out_ssseq(titr) = HMMSTR%SS_seq(Q(titr))
      !out_ramaseq(titr) = HMMSTR%RAMA_seq(Q(titr))
      !out_tmseq(titr) = HMMSTR%TM_seq(Q(titr))
   enddo
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! TLB 9/8/2021 moved from diagnostics.py to hmm_gamma.f95 !!!!
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! TLB 9/8/2021 moved from mda.f95 to hmm_gamma.f95 !!!!
  ! TLB 9/8/2021 moved from hmm_gamma.f95 to hmmstrtm.f95 !!!!!!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 subroutine get_mda(profile, paradigm, mda, t, t_mda)
  ...
  logical function rama_comp(rama_char, rama_ground) result(ret)
      ...
      subroutine gamma_voting(t, HMMSTR1, xgamma1, seq1, ssseq1, ramaseq1, tmseq1)
             implicit none
             integer,intent(in) :: t
             Type(HMM),intent(in) :: HMMSTR1
      ...
      do titr=1,t
                ...
                do nitr=HMMSTR1%k + 1,HMMSTR1%n
                    aa_prof    = aa_prof  + xgamma1(titr,nitr)*HMMSTR1%AA(nitr,:)
                    ss_prof    = ss_prof  + xgamma1(titr,nitr)*HMMSTR1%SS(nitr,:)
                    tm_prof    = tm_prof  + xgamma1(titr,nitr)*HMMSTR1%TM(nitr,:)
                    rama_prof  = rama_prof  + xgamma1(titr,nitr)*HMMSTR1%RAMA(nitr,:)
                enddo
                call voting(seq1, tmseq1, ssseq1, ramaseq1, aa_prof, tm_prof, ss_prof, rama_prof, titr)
