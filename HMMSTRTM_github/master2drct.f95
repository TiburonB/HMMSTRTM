program master2drct
    ...
    integer :: jarg
    integer*4 :: N
    character(len=5000) :: SSseq, TMseq
    character(len=5000) :: AAseq
    ...
    offset = 0
    if (jarg < 6) then
        write(*,*) 'Usage: xprofile2drct seq ssseq tmseq profilefile codechain outfile'
        stop
    else
        call getarg(1, AAseq)
        call getarg(2, SSseq)
        call getarg(3, TMseq)
        call getarg(4, profilefile)
        call getarg(5, code_chain)
    ...
    do
        ...
        nrec = nrec + 1
        sschar = SSseq(nrec:nrec) 
        ramachar = ' '
        mrchar = TMseq(nrec:nrec) 
...
!  # $template = "s s A4 A A A A f f f f f f f f f f f f f f f f f f f f f f f f f f f f f A A s3";
        write(2, rec = nrec) iseq, iseq, code, chainID, aa, sschar, rotamer, &
                   calpha, dih, csum, profile, 0., 0., ramachar, mrchar, clusterID
   enddo
