...
Beta_emission =  [ 0.1, 0.01, 0.83, .01, 0.01, 0.01, 0.01, 0.01, 0.01, 0 ]
node_data = { "AA": CANON.AA_ground, "RAMA": CANON.RAMA_ground, "SS": CANON.SS_ground, "TM": Beta_emission }
data_dict = datamine('./tm_training.drct', code = 'null') # grab data_dict of the database.
distribution = get_period(data_dict, 'TMSEQ', 'B')
view_np(distribution,'TMB_periodicity', './', QUIET = False, nf = max(distribution)-min(distribution))
make_TM_B_region(CANON, node_data, distribution)

CANON.AA_EPS = 0.00001
CANON.SS_EPS = 0.00001
CANON.TM_EPS = 0.00000000001
CANON.RAMA_EPS = 0.00000001
CANON.write('./TMBTOP/HMMSTRTM.hmm')
CANON = hmm('HMMSTRTM.hmm', directory = './TMBTOP/')
CANON.read()
display_by_TM(CANON)
